...
def train(
    ...
    for epoch in range(epochs):
        ...
        loader_with_progress = tqdm(loader, desc=f"Epoch {epoch+1}/{epochs}", leave=True)
        for i, (inp, pos) in enumerate(loader_with_progress):
            ...
            # Put this right before the loop restarts for the next batch
            loader_with_progress.set_postfix({"Loss": f"{(loss_sum / loss_count):.4f}"})

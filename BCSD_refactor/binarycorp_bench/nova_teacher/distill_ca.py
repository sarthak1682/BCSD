...
ADAPTER_PATH = os.path.abspath(os.path.join(script_dir_file, "../nova_teacher/nova_contrastive_bidir_noMNTP_bench_ca"))
...
# Fraction of positive pairs that are forced to span two different architectures
# (when the function has variants in >=2 archs). This injects the cross-arch
# gradient signal that plain round-robin pairing almost never produced.
...
def build_bench_pairs(grouped, func_ids, func_id_to_int, seed=42, cross_arch_prob=CROSS_ARCH_PAIR_PROB):
    """Build positive (query, positive, func_int) pairs for contrastive distillation.
    ...
    archs. For functions that exist in >=2 architectures we deliberately route most queries
    to a positive drawn from a *different* architecture; the rest fall back to round-robin
    ...
    rng.shuffle(pairs)
    log(
        f"Built {len(pairs)} pairs from {len(func_ids)} functions "
...
log("Copying teacher embedding into student and freezing it...")
teacher_emb = teacher_model.get_input_embeddings().weight.detach().clone()
student_model.embedding.weight.data.copy_(teacher_emb)
student_model.embedding.requires_grad_(False)
